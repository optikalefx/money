...
<svelte:head>
	<title>Expected · Money Tracker</title>
	<meta name="description" content="Review and manage merchants and categories marked as expected." />
</svelte:head>
...
<main class="money-shell">
	<section class="page-heading">
		<div>
			...
			<h1>Expected spending</h1>
			<p class="lede">
				Every merchant and category you marked as expected, summed across all time. Unmark one to
				send it and its transactions back to dynamic.
			</p>
		</div>
	</section>
	...
	<section class="summary-grid">
		<div class="organic-card">
			<span class="metric-label">Expected monthly</span>
			<strong>{formatAmount(summaryData?.monthlyTotal ?? 0)}</strong>
			<p>Average per month across every expected rule</p>
		</div>
		<div class="organic-card">
			<span class="metric-label">Expected merchants</span>
			<strong>{summaryData?.merchantCount ?? 0}</strong>
			<p>Distinct merchants marked expected</p>
		</div>
		<div class="organic-card">
			<span class="metric-label">Expected categories</span>
			<strong>{summaryData?.categoryCount ?? 0}</strong>
			<p>Provider categories marked expected</p>
		</div>
	</section>
				...
				{#each byMerchant as row (row.key)}
					<div class="merchant-row">
						...
						<button
							...
							onclick={() => unmarkMerchant(row)}
						>
							{unmarkingKey === row.key ? 'Unmarking...' : 'Unmark'}
						</button>
					</div>
				{:else}
					<div class="empty-state">No expected merchants yet.</div>
				{/each}
			</div>
		</div>
				...
				{#each byCategory as row (row.key)}
					<div class="merchant-row">
						...
						<button
							type="button"
							class="unmark-button"
							title="Move this category and its transactions back to dynamic"
							disabled={unmarkingKey === row.key}
							onclick={() => unmarkCategory(row)}
						>
							{unmarkingKey === row.key ? 'Unmarking...' : 'Unmark'}
						</button>
					</div>
				{:else}
					<div class="empty-state">No expected categories yet.</div>
				{/each}
			</div>
		</div>
	</section>
	...
	<section class="data-section">
		...
		<div class="table-shell organic-surface">
			{#if table.isLoading}
				<div class="empty-state">Loading expected transactions...</div>
			{:else if table.error}
				<div class="empty-state">Unable to load expected transactions.</div>
			...
			{:else}
				<div class="empty-state">No transactions marked as expected in this range.</div>
			{/if}
		</div>
	</section>
</main>
